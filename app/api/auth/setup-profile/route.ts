...
// Called when the user has an auth account but no profile row in public.users
export async function POST(req: Request) {
  try {
    ...
    if (upsertError) {
      console.error("Setup profile error:", upsertError);
      return NextResponse.json({ error: `DB error: ${upsertError.code} — ${upsertError.message}` }, { status: 500 });
    }
    ...
    console.error("Setup profile unhandled:", err);
